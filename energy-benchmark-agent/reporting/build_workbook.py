...
def _sheet_calcoli(wb: Workbook, result: RelampingResult) -> int:
    ...
    cols = [
        "Edificio/zona", "Zona", "Tipo di illuminazione", "n. lampade",
        "Potenza lampada ANTE [kW]", "Potenza totale ANTE [kW]",
        "Potenza lampada POST [kW]", "Costo lampada [€]", "Costo totale [€]",
        "Potenza totale POST [kW]", "h eq.", "Consumo ANTE [kWh]",
        "Consumo POST [kWh]", "Spesa ANTE [€]", "Spesa POST [€]",
        "Risparmio EE [kWh]", "Risparmio € [€]", "Risparmio CO2 [kg]",
    ]
    _hdr(ws, 1, cols)

    for idx, riga in enumerate(result.righe):
        r = 2 + idx
        h_eq = riga.h_equivalenti * riga.fatt_contemporaneita

        ws.cell(row=r, column=2, value=riga.zona)                  # B
        ws.cell(row=r, column=3, value=riga.pod)                  # C
        ws.cell(row=r, column=4, value=riga.tipologia_originale)  # D
        # input (giallo)
        e = ws.cell(row=r, column=5, value=riga.n_utenze)            # E n
        f = ws.cell(row=r, column=6, value=riga.p_lampada_ante_kw)   # F P ante
        h = ws.cell(row=r, column=8, value=riga.p_lampada_post_kw)   # H P post
        i = ws.cell(row=r, column=9, value=riga.costo_lampada_euro)  # I costo
        l = ws.cell(row=r, column=12, value=round(h_eq, 4))          # L h eq.
        for cc in (e, f, h, i, l):
            cc.fill = _FILL_INP
        # formule
        ws.cell(row=r, column=7,  value=f"=F{r}*E{r}")                       # G P tot ante
        ws.cell(row=r, column=10, value=f"=I{r}*E{r}")                       # J costo tot
        ws.cell(row=r, column=11, value=f"=H{r}*E{r}")                       # K P tot post
        ws.cell(row=r, column=13, value=f"=G{r}*L{r}")                       # M consumo ante
        ws.cell(row=r, column=14, value=f"=L{r}*K{r}")                       # N consumo post
        ws.cell(row=r, column=15, value=f"=M{r}*'{FATT}'!$C$24")            # O spesa ante
        ws.cell(row=r, column=16, value=f"=N{r}*'{FATT}'!$C$24")            # P spesa post
        ws.cell(row=r, column=17, value=f"=M{r}-N{r}")                       # Q risparmio EE
        ws.cell(row=r, column=18, value=f"=O{r}-P{r}")                       # R risparmio €
        ws.cell(row=r, column=19, value=f"=(M{r}-N{r})*'{FATT}'!$C$15/1000")# S risparmio CO2

    last = 1 + len(result.righe)

    # blocco sommario in V/W (col 22/23)
    somm = [
        ("Consumo EE Ante [kWh]", f"=SUM(M2:M{last})"),
        ("Consumo EE Post [kWh]", f"=SUM(N2:N{last})"),
        ("Capex [€]",             f"=SUM(J2:J{last})"),
        ("Spesa Ante",            f"=SUM(O2:O{last})"),
        ("Spesa Post",            f"=SUM(P2:P{last})"),
    ]
    for k, (lab, formula) in enumerate(somm):
        r = 4 + k
        cl = ws.cell(row=r, column=22, value=lab)
        cl.font = _F_BOLD
        cv = ws.cell(row=r, column=23, value=formula)
        cv.fill = _FILL_TOT
        cv.font = _F_BOLD

    # larghezze
    widths = [16, 18, 22, 10, 12, 12, 12, 11, 11, 12, 9, 13, 13, 12, 12, 13, 12, 13]
...
def _sheet_dati(wb: Workbook, result: RelampingResult, last: int) -> None:
    ...
    riga(5,  "Consumo EE PRE INTERVENTO", "[kWh]", f"={CALC}!W4", _FMT_KWH)
    riga(6,  "Consumo EE POST INTERVENTO", "[kWh]", f"={CALC}!W5", _FMT_KWH)
    ...
    riga(13, "Investimento iniziale", "[€]", f"={CALC}!W6", _FMT_EUR)
